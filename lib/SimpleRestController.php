...
require_once 'SimpleRestAPI.php';
...
class SimpleRestController {
	...
	public function __construct( $userSettings = array() ) {
        ...
        $this->environment = SimpleRestEnvironment::getInstance();
		$this->request = new SimpleRestHttpRequest($this->environment);
		$this->response = new SimpleRestHttpResponse();
		$this->view = new SimpleRestView();
	...
	public function run(){
		...
		try{
			...
			if(in_array('Auth',$user_class->uses) ) {
				$this->auth = new SimpleRestAuth(DATABASE_CONFIG::default_db());
        ...
        if ( headers_sent($filename, $linenum) === false ) {
            ...
            if ( strpos(PHP_SAPI, 'cgi') === 0 ) {
                header(sprintf('Status: %s', SimpleRestHttpResponse::getMessageForCode($status)));
            } else {
                header(sprintf('HTTP/1.1 %s', SimpleRestHttpResponse::getMessageForCode($status)));
            }
...
$SimpleRest = new SimpleRestController();
$SimpleRest->run();
